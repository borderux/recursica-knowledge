#!/usr/bin/env node
/**
 * Nest-side launcher for the published-text name guard.
 *
 ...
 * alternative is denying every git command on the machine, which gets the hook removed
 * within the hour — and a guard that has been switched off protects nothing. The
 * commit-msg hook inside the checkout is unaffected either way.
 */
...
const REL = path.join("buzz-agents", "scripts", "hook-guard-published-text.mjs");
...
if (!guard) {
  // Say so once, on a command that was going to publish something, rather than silently
  // waving it through — a silent no-op is how the last guard went unnoticed for months.
  try {
    const cmd = JSON.parse(payload)?.tool_input?.command ?? "";
    if (/\b(git\s+(commit|tag|branch|switch|checkout)|gh\s+(pr|issue|release)|api\.github\.com|buzz\s+(pr|issues))\b/.test(cmd)) {
      process.stdout.write(
        JSON.stringify({
          systemMessage:
            `Name guard is NOT running: no knowledge checkout found under ${path.join(BUZZ_HOME, "REPOS")}. ` +
            `Text published by this command is unchecked. Clone the repository there, or run ` +
            `check-text-for-names.mjs by hand before publishing.`,
        }),
      );
    }
  } catch {
