...
4. Generate a 'participant*id' for each unique speaker (e.g., prefix 'p*' + lowercase speaker name with alphanumeric characters only, spaces replaced with underscores). Always use snake_case with underscores for participant_id (e.g. p_first_last). Never omit underscores between name parts.
...
- Direct Text Corrections: Replace typos directly (e.g., "Acme Corp" -> "ACME").
