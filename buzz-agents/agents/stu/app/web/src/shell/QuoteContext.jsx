...
// `recursica-skill-panels-modals` sends content that induces vertical scrolling out of a panel
// and onto a page. So this shows the anchored line and a few turns either side, and choosing a
// different quote *re-anchors* the window instead of scrolling a whole transcript inside it. The
// full transcript is a page, and the footer goes there.
//
...
import { Avatar, Badge, Button, Group, Link, Panel, Stack, Text } from '@recursica/mantine-adapter'
import { api } from '../api.js'

/** Turns either side of the cited line. Eleven lines is what the panel holds without scrolling. */
const RADIUS = 5
...
export function QuoteContext({ citation, siblings = [], onAnchor, onClose }) {
  ...
  // Opening must be perceivable, which follows from focus moving into the panel — the slide-in
  // is not allowed to be the only signal. Focus returns to the trigger on close via Mantine's
          ...
          {others.length > 0 && (
            <Stack gap="xs">
              <Text variant="caption">
                The other {others.length === 1 ? 'line' : `${others.length} lines`} this claim rests on
              </Text>
              {/* Re-anchors the window. FR-16: choosing a quote moves the panel to that point —
                  here by moving the window, since the panel holds no scroll region to move within. */}
              <Stack gap={4}>
                {others.map((s) => (
                  <div key={s.line_id}>
                    <Button variant="text" size="small" onClick={() => onAnchor(s)}>
                      Line {String(s.line_sequence_number)} — “{truncate(s.quote)}”
                    </Button>
                  </div>
                ))}
              </Stack>
