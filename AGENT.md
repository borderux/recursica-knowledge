...
| `circlechat/`                                                 | A third deployment surface for the same agents: setup scripts and CircleChat persona text for running Betty, Barb and Norm on a self-hosted CircleChat with the Hermes runtime. Operational tooling, not design guidance. Each persona is composed at install from the agent's built artifact in `portable/claude-code/agents/` plus `circlechat/souls/<handle>.md`, so `agents/<name>/` stays the source of truth and the soul file carries only what is true on this surface. See [circlechat/README.md](circlechat/README.md).                                |
