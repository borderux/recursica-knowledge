...
| `circlechat/`                                                 | A third deployment surface for the same agents: setup scripts and CircleChat persona text for running Betty, Barb and Norm on a self-hosted CircleChat with the Hermes runtime. Operational tooling, not design guidance. Barb's persona is composed at install from `agents/barb/SKILL.md` plus `circlechat/souls/barb.md`, so her definition is read rather than copied. Betty's and Norm's are the soul file alone — Betty has an `agents/betty/` that this surface does not draw on, and Norm has no `agents/` entry at all. See [circlechat/README.md](circlechat/README.md). |
