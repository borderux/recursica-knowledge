...
| `buzz-agents/`                                                | Versioned definitions of the Buzz agents themselves — system prompts, settings, and avatars, kept so they can be branched and rebuilt in a new Buzz community. Configuration, not design guidance. See [buzz-agents/README.md](buzz-agents/README.md), [buzz-agents/INSTALL.md](buzz-agents/INSTALL.md) to set them up on a machine, or [buzz-agents/ONBOARD_AN_OPERATOR.md](buzz-agents/ONBOARD_AN_OPERATOR.md) for the owner's side of handing them to a teammate. |
