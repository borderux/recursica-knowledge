...
- **An `image` avatar needs alternative text naming the person or thing** whenever the avatar is the only place that name appears. "Aaron Martlage", not "avatar" and not "profile photo".
- **When the name is already adjacent in text, the avatar is decorative and must be silent.** Give it empty alternative text. "Aaron Martlage, image, Aaron Martlage" is worse than saying nothing, and it happens in every row of the list.
