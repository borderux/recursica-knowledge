...
- **Whether a carousel exists in the component inventory.** It is now cited twice as house-sanctioned — the only component permitted horizontal movement, and the mobile replacement for a card set — but there is no `recursica-skill-carousel` and no entry in the token inventory.
- **What tablet actually looks like.** Rules exist for below-tablet and for the small-device tier; tablet itself is largely undescribed, as is whether the two narrow tiers each get their own design.
...
| Whether a scrim is ever drawn behind a panel                 | Never. A panel is semi-modal and exists so the page behind stays readable; Mantine's and MUI's default overlay is a library default, not a house rule                                                 | `recursica-skill-panels-modals`, `recursica-skill-panel`               |
| The breakpoint values                                        | Desktop 1200, tablet 1024, small device 400. 1200 doubles as the max width of the main content area; chrome may span wider                                                                            | `recursica-skill-responsive-behavior`                                  |
| Whether anything besides a panel becomes a page below tablet | A work-bearing modal does — a mini workflow, form, or detail edit. A confirmation modal stays a modal at every width                                                                                  | `recursica-skill-responsive-behavior`, `recursica-skill-panels-modals` |
| Whether a table may scroll horizontally on a narrow screen   | The question does not arise: a complex data table is not displayed below tablet at all                                                                                                                | `recursica-skill-responsive-behavior`, `recursica-skill-tables`        |
| Whether a bottom navigation bar is allowed on a phone        | No. Raised as an alternative to the hamburger and rejected — it is not a house pattern at any width                                                                                                   | `recursica-skill-responsive-behavior`, `recursica-skill-navigation`    |
