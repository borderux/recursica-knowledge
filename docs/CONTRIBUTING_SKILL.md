...
- `skills/design-rules/` — one skill per design topic (`recursica-skill-forms`, `recursica-skill-navigation`, `recursica-skill-buttons-links`, `recursica-skill-selection-controls`, `recursica-skill-badges-chips`, `recursica-skill-data-visualization`, `recursica-skill-dashboards`, `recursica-skill-tables`, `recursica-skill-system-conventions`). These carry the team's opinions as rules a build agent must follow, and are usually derived from a recorded design interview.
